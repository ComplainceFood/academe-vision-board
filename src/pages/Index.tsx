  ...
  return (
    <MainLayout>
      <div className="animate-fade-in space-y-4">
        ...
        {/* Welcome Banner */}
        <div className="relative overflow-hidden rounded-xl bg-gradient-to-br from-primary via-primary to-secondary p-4 sm:p-6 text-primary-foreground shadow-lg">
          ...
          <div className="relative z-10 flex flex-col sm:flex-row sm:items-center justify-between gap-2 sm:gap-3">
            <div className="min-w-0">
              <div className="flex items-center gap-2 mb-1">
                ...
                <span className="text-sm text-primary-foreground/80 truncate">{today}</span>
              </div>
              <h1 className="text-xl sm:text-2xl font-bold tracking-tight leading-tight">Welcome back!</h1>
              <p className="text-primary-foreground/75 text-sm mt-1">Your academic overview for today</p>
            </div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-2.5 sm:gap-3">
          {statCards.map(({ title, value, icon: Icon, to, desc, alert }) => (
            <Link key={title} to={to} className="group">
              <Card className={`h-full border bg-card shadow-sm transition-all duration-200 group-hover:shadow-md group-hover:-translate-y-0.5 ${alert ? 'border-destructive/30' : ''}`}>
                <CardContent className="p-3.5 sm:p-4">
                  <div className="flex items-start justify-between mb-2.5 sm:mb-3">
                    <div className={`p-1.5 rounded-lg ${alert ? 'bg-destructive/10' : 'bg-primary/10'}`}>
                      <Icon className={`h-4 w-4 ${alert ? 'text-destructive' : 'text-primary'}`} />
                    </div>
                    {alert && (
                      <span className="text-xs bg-destructive/10 text-destructive px-1.5 py-0.5 rounded-full font-medium">!</span>
                    )}
                  </div>
                  <p className="text-2xl font-bold text-foreground leading-none">{value}</p>
                  <p className="text-xs text-muted-foreground mt-1.5 leading-tight line-clamp-2">{title}</p>
                </CardContent>
              </Card>
            </Link>
          ))}
        ...
        {/* Three-column panel: Events | Quick Actions | All Modules (stacks on tablet) */}
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-[1.4fr_0.9fr_2fr] gap-3">
                  ...
                  {weekEvents.slice(0, 7).map((event: any) => {
                    ...
                    return (
                      <div key={event.id} className="flex items-center justify-between px-3 py-2 rounded-md bg-muted/50 hover:bg-muted transition-colors">
                        ...
                        <Badge variant={event.priority === 'high' ? 'destructive' : event.priority === 'medium' ? 'default' : 'secondary'} className="text-xs px-1.5 ml-2 shrink-0">
                          {event.priority || 'low'}
                        </Badge>
                      </div>
                    );
