...
import { Plus, Mic, ExternalLink, Edit, Trash2 } from "lucide-react";
...
export function InvitedTalksList() {
  ...
  const { data: talks, isLoading, refetch } = useDataFetching<InvitedTalk>({
      ...
      {!talks?.length ? (
        <Card>
          <CardContent className="p-6 text-center">
            ...
            <h3 className="font-semibold mb-2">No invited talks yet</h3>
            <p className="text-muted-foreground mb-4">
              Track your invited talks and oral presentations
            </p>
            ...
            <Card key={talk.id}>
              ...
              <CardContent className="pt-0">
                ...
                <div className="flex flex-wrap gap-4 text-sm text-muted-foreground">
                  {talk.date && (
                    <span>Date: {new Date(talk.date).toLocaleDateString()}</span>
                  )}
                </div>
